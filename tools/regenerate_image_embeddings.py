    ...
    with SessionLocal() as session:
        ...
        for embedding_row, header_row in rows:
            ...
            try:
                ...
                with torch.no_grad():
                    text_vec = F.normalize(model.encode_text(clip.tokenize([text_prompt]).to(device)), dim=-1).squeeze(0).cpu().numpy().tolist()
...
def update_image_embedding_common_names(session, image_records):
    ...
    for embedding_row in image_records:
        ...
        with torch.no_grad():
            text_vec = F.normalize(model.encode_text(clip.tokenize([f"a photo of a {best_common_name.lower()}"]).to(device)), dim=-1).squeeze(0).cpu().numpy().tolist()
